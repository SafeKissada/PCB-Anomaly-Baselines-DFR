...
def plot_training_history(history: dict, cfg) -> str:
    """กราฟ training history ของ DFR — 1 เส้น (train_loss, normal only)

    ต่างจาก repo หลัก (EXPERIMENT 0) ที่มี 4 แผง (train_loss, val_loss,
    val_loss_normal, val_auroc) ตรงที่ DFR.fit() ตาม BaseAnomalyModel
    interface รับแค่ normal_loader ไม่มี val_loader ให้ monitor ระหว่าง
    เทรนเลย จึงไม่มี val curve ให้พล็อต — ไม่ใช่บั๊ก แต่เป็นข้อจำกัดเชิง
    สถาปัตยกรรมของ interface ที่ทุก baseline (PatchCore/PaDiM/DRAEM/
    SimpleNet/RD4AD/DFR) ต้องเคารพร่วมกัน (ดู README หัวข้อ
    "ความต่างจาก repo หลัก")

    Plots DFR's training history — a single curve (train_loss, normal
    only). Unlike the main repo (EXPERIMENT 0), which has 4 panels
    (train_loss, val_loss, val_loss_normal, val_auroc), DFR.fit() per the
    BaseAnomalyModel interface only receives normal_loader — there is no
    val_loader to monitor during training, hence no val curve to plot.
    This is not a bug but an architectural constraint every baseline
    (PatchCore/PaDiM/DRAEM/SimpleNet/RD4AD/DFR) shares under this
    interface (see README, "Differences from the main repo").
    """
    fig, ax = plt.subplots(1, 1, figsize=(7, 5))
    train_loss = history['train_loss']
    best_ep = int(np.argmin(train_loss))
    ax.plot(train_loss, label='Train Loss (normal only)', color='#2196F3', lw=2)
    ax.axvline(best_ep, color='black', linestyle=':', alpha=0.6,
               label=f'Min loss epoch {best_ep + 1}\n(loss={train_loss[best_ep]:.6f})')
    ax.set_title('DFR CAE Training Loss (train-normal only, no val monitoring)',
                 fontsize=12, fontweight='bold')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Reconstruction Loss (L2)')
    ax.legend(fontsize=9)
    ax.grid(alpha=0.3)
    ax.spines[['top', 'right']].set_visible(False)

    plt.tight_layout()
    out = f'{cfg.OUTPUT_PATH}/training_history.png'
    plt.savefig(out, dpi=150, bbox_inches='tight')
    plt.show()
    return out
